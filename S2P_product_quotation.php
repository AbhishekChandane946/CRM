                  ...
                  <table class="table " id="quotationTable">
                    ...
                    <tbody>
                        ...
                        <tr class="product-row">
                            ...
                            <td>
                                <input type="text" class="form-control productPrice" readonly>
                            </td>
                            <td>
                                <input type="number" class="form-control salesInput" value="0">
                            </td>
                            <td>
                                <!-- Add New Row Button -->
                                <button type="button" class="btn btn-primary btn-icon" id="addRow">
                                ...
                                <!-- Remove Row Button -->
                                <button type="button" class="btn btn-danger btn-icon removeRow">
...
<script>
    $(document).ready(function () {
        ...
        // Fetch products from API and store in productsData
        function fetchProducts() {
            $.ajax({
                ...
                success: function (data) {
                    productsData = data;
                    populateProductDropdown($(".productSelect")); // Populate only once on initial load
                },
        ...
        function populateProductDropdown(selectBox) {
            selectBox.html('<option value="">-- Select Product --</option>'); // Reset options

            $.each(productsData, function (index, product) {
                selectBox.append(`<option value="${product.price}" data-price="${product.price}">${product.name}</option>`);
            });
        }

        // When a product is selected, update price and prefill sales amount
        $(document).on("change", ".productSelect", function () {
            let selectedPrice = $(this).find(":selected").data("price");
            let row = $(this).closest("tr");

            row.find(".productPrice").val(selectedPrice || 0); // Set price
            row.find(".salesInput").val(selectedPrice || 0);   // Prefill sales amount (editable)

            calculateTotal();
        });

        // When sales amount is updated, recalculate total
        $(document).on("input", ".salesInput", function () {
            calculateTotal();
        });
        ...
        $("#addRow").click(function () {
            ...
            populateProductDropdown(newRow.find(".productSelect")); // Populate only the new row
        });
        ...
        function calculateTotal() {
            ...
            $(".product-row").each(function () {
                ...
                let salesAmount = parseFloat($(this).find(".salesInput").val()) || 0;
                total += price * salesAmount;
            });
