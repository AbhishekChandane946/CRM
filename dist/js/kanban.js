...
function getLeads(statuses) {
    $.ajax({
        ...
        error: function () {
            Swal.fire({
                icon: "error",
                title: "Error fetching data from the server.",
            });
        }
    });
}
        ...
        $(this).dxSortable({
            ...
            onAdd: function (e) {
                ...
                if (!lead_id || !newStatus) {
                    Swal.fire({
                        title: "Lead or new status not assigned",
                        icon: "success",
                    });
                    e.component.cancel();
                    return;
...
function renderCard($container, lead) {
    ...
    $('<div>').addClass('fw-bold text-primary').attr('title', `Lead Name: ${lead.lead_name}`)
        ...
        .appendTo($nameRow).on('click', function (event) {
            ...
            window.location.href = `S2P_view_lead.php?lead_id=${lead.lead_id}`;
        });
    // $('<div>')
